...
RSpec.feature 'Post show page', type: :feature do
  before do
    ...
    @first_comment = Comment.create(user_id: @first_user.id, post_id: @first_post.id,
                                    text: 'This the first comment for first post')
    @second_comment = Comment.create(user_id: @second_user.id, post_id: @first_post.id,
                                     text: 'This the second comment for first post')
    @third_comment = Comment.create(user_id: @third_user.id, post_id: @first_post.id,
                                    text: 'This the third comment for first post')
    @fourth_comment = Comment.create(user_id: @first_user.id, post_id: @first_post.id,
                                     text: 'This the fourth comment for first post')
    @fifth_comment = Comment.create(user_id: @first_user.id, post_id: @first_post.id,
                                    text: 'This the fifth comment for first post')
    @six_comment = Comment.create(user_id: @first_user.id, post_id: @first_post.id,
                                  text: 'This the sixth comment for first post')

    Like.create(user_id: @first_user.id, post_id: @first_post.id)
    Like.create(user_id: @second_user.id, post_id: @first_post.id)
    Like.create(user_id: @third_user.id, post_id: @first_post.id)
  ...
  scenario 'should show post user name' do
